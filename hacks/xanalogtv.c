...
update_smpte_colorbars(analogtv_input *input)
{
  ...
  */
  ...
  if (background == 1)
    analogtv_draw_solid_rel_lcp(input,  0.05,0.9,  0.5,0.63,  15, 31, 247);
...
xanalogtv_event (Display *dpy, Window window, void *closure, XEvent *event)
{
  ...
  else if (event->type == KeyPress)
  {
    ...
    else if (event->xkey.keycode>=0x43 && event->xkey.keycode<=0x45)
    {
      welcamline = event->xkey.keycode - 0x43;
      welcampos = 0;
    }
    else if (event->xkey.keycode>=0x47 && event->xkey.keycode<=0x49)
    {
      background = event->xkey.keycode - 0x47;
    }
  }
